...
class CourierOptimizer:
    ...
    TRANSPORT_PARAMS = {
        "CAR": {"speed_kmh": 50, "cost_per_km": 4.0, "co2_g_per_km": 120},
        "BICYCLE": {"speed_kmh": 15, "cost_per_km": 0.0, "co2_g_per_km": 0},
        "WALKING": {"speed_kmh": 5, "cost_per_km": 0.0, "co2_g_per_km": 0},
    }
    ...
    def validate_delivery(self, delivery: Dict[str, Any]) -> Dict[str, Any]:
        ...
        all_warnings.extend(
            self._validate_coordinates(
                delivery.get("latitude", 0), delivery.get("longitude", 0)
            )
        )
        all_warnings.extend(self._validate_customer_name(delivery.get("customer", "")))

        return {"is_valid": len(all_warnings) == 0, "warnings": all_warnings}
    ...
    def process_csv_data(self, data) -> Dict[str, List[Dict]]:
        ...
        self.logger.info(
            f"Validation complete: {len(valid_deliveries)} valid, {len(invalid_deliveries)} invalid"
        )
    ...
    @timer
    def optimize_route(
        self, deliveries: List[Dict], transport_mode: str, criteria: str
    ) -> List[Dict]:
        """
        ...
        sorted_deliveries = sorted(
            ...
            key=lambda d: (
                ...
                self.calculate_distance(
                    depot_lat, depot_lon, d["latitude"], d["longitude"]
                ),
            ),
        )
    ...
    def calculate_route_metrics(
        self, route: List[Dict], transport_mode: str
    ) -> Dict[str, float]:
        """
    ...
    def write_route_csv(
        ...
        print(
            f"   Total time: {metrics['total_time_hours']} hours ({metrics['total_time_hours']*60:.0f} minutes)"
        )
        print(f"   Total cost: {metrics['total_cost_nok']} NOK")
        print(
            f"   Total CO2: {metrics['total_co2_grams']} grams ({metrics['total_co2_grams']/1000:.2f} kg)"
        )
