...
class PygameVisualizer:
    ...
    def _draw_ui(self) -> None:
        ...
        # Controls hint
        controls_text = "SPACE: Play/Pause | C: Clear | B: Blinker | G: Glider | ESC: Exit"
    ...
    def _load_pattern(self, pattern_name: str) -> None:
        ...
        success = PatternLibrary.create_pattern(pattern_name, self.game, center_x, center_y)
        
        if success:
            pattern_info = PatternLibrary.get_pattern_info().get(pattern_name, {})
            print(f"✨ Loaded {pattern_info.get('name', pattern_name)} pattern at ({center_x}, {center_y})")
            # Show initial state
            alive_count = self._count_alive_cells()
