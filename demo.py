...
def demo_validation():
    ...
    print(f"   Priority: {valid_delivery['priority']}, Weight: {valid_delivery['weight_kg']}kg")
    ...
    print(f"   Location: ({invalid_delivery['latitude']}, {invalid_delivery['longitude']})")
    print(f"   Priority: {invalid_delivery['priority']}, Weight: {invalid_delivery['weight_kg']}kg")
...
def demo_distance_calculation():
    ...
    print(f"   City Hall → Opera House: {distance1:.2f} km (~{distance1*1000:.0f} meters)")
...
def demo_route_optimization():
    ...
    deliveries = [
        {'customer': 'Kaia', 'latitude': 59.95, 'longitude': 10.75, 
         'priority': 'LOW', 'weight_kg': 5},
        {'customer': 'Riley', 'latitude': 59.85, 'longitude': 10.70, 
         'priority': 'HIGH', 'weight_kg': 3},
        {'customer': 'Kairo', 'latitude': 59.92, 'longitude': 10.85, 
         'priority': 'HIGH', 'weight_kg': 7},
        {'customer': 'Elijah', 'latitude': 59.88, 'longitude': 10.65, 
         'priority': 'MEDIUM', 'weight_kg': 2},
    ]
    
    print("📦 Original delivery order:\n")
    for i, d in enumerate(deliveries, 1):
        dist = optimizer.calculate_distance(59.9114, 10.7343, d['latitude'], d['longitude'])
        print(f"   {i}. {d['customer']:10} - Priority: {d['priority']:6} - "
    ...
    for i, d in enumerate(route, 1):
        dist_from_depot = optimizer.calculate_distance(59.9114, 10.7343, 
                                                       d['latitude'], d['longitude'])
        dist_from_prev = optimizer.calculate_distance(prev_lat, prev_lon,
                                                      d['latitude'], d['longitude'])
    ...
    for mode in ['CAR', 'BICYCLE', 'WALKING']:
        ...
        print(f"      Time: {metrics['total_time_hours']} hours ({metrics['total_time_hours']*60:.0f} minutes)")
        print(f"      Cost: {metrics['total_cost_nok']} NOK")
        print(f"      CO2: {metrics['total_co2_grams']} g ({metrics['total_co2_grams']/1000:.2f} kg)")
        print()
...
def demo_csv_processing():
    ...
    sample_data = pd.DataFrame([
        {'customer': 'Kairo', 'latitude': 59.9139, 'longitude': 10.7522, 
         'priority': 'HIGH', 'weight_kg': 15},
        {'customer': 'Riley', 'latitude': 59.9200, 'longitude': 10.7500, 
         'priority': 'LOW', 'weight_kg': 5},
        {'customer': 'InvalidGuy', 'latitude': 60.5, 'longitude': 11.0, 
         'priority': 'INVALID', 'weight_kg': 30},
        {'customer': 'Raikko', 'latitude': 59.88, 'longitude': 10.70,
         'priority': 'MEDIUM', 'weight_kg': 10}
    ])
