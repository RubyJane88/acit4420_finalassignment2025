...
def process_deliveries(transport_mode, criteria):
    ...
    try:
        ...
        print(f"⏱️  Total time: {metrics['total_time_hours']:.2f} hours ({metrics['total_time_hours']*60:.0f} minutes)")
        print(f"💰 Total cost: {metrics['total_cost_nok']:.2f} NOK")
        print(f"🌍 Total CO2: {metrics['total_co2_grams']:.2f} g ({metrics['total_co2_grams']/1000:.2f} kg)")
        print(f"\n📂 Output files:")
        print(f"   • {route_file}")
